...
double LshapeTracker::smoothAngle(const double &ang, bool &use_last) {
  if(angle_history.size() == 0){
    angle_history.push_front(ang);
    use_last = false;
    return ang;
  }
  // double mean_angle = 0.0;
  // for (const auto& angle : angle_history)
  //     mean_angle += angle;
  // mean_angle = mean_angle / angle_history.size();

  if(abs(angle_history[0] - ang) >= 1.8 && angle_history.size() != 0){
    angle_history.push_front(angle_history[0]); 
    if (angle_history.size() > history_size)
      angle_history.pop_back();
    use_last = true;
    return angle_history[0];
  }
  else{
    angle_history.push_front(ang);
    if (angle_history.size() > history_size)
...
void LshapeTracker::findOrientation(double& psi, double& length, double& width){
  ...
  // bool use_last = false;
  // orientation = smoothAngle(orientation, use_last);
  ...
  // if(use_last && (last_length!=0 && last_width!=0)){
  //   length = last_length;
  //   width = last_width;
  // }
  // last_length = length;
